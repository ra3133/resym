...
def align_heap_access(fname, align_data, field_access_data) -> List[Dict]:
    ...
    for access in field_access_data:
        if int(access['offset']) == 0 and access['expr'] not in expr_selected_zero_offset:
            continue 
        if access['varName'] in seen:
            continue
        try:
            it_save_data = _align_heap_access_helper(access)
            save_data.append(it_save_data)
            seen.add(access['varName'])
        except VarAlignException as e:
            print(f"{fname} - {access['varName']} - {e.msg}")
            continue
...
def main(align_folder, filed_access_folder, align_data_save_dir, train_data_save_dir, target_bin):
    success_cnt = 0
    fail_cnt = 0
    unavailable = 0
    ...
    for f in tqdm(get_file_list(align_folder), disable=target_bin):
        ...
        try:
            ...
            if not os.path.exists(os.path.join(filed_access_folder, f)):
                unavailable += 1
                print(f"Cannot find the field access info file in {os.path.join(filed_access_folder, f)}")
                continue
...
if __name__=='__main__':
    ...
    main(args.align_dir, args.filed_access_folder, args.align_data_save_dir, args.train_data_save_dir, target_bin = args.bin)
